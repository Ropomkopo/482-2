...
class ModelToolSeoPackageEditor extends Model {
	...
	public function filter ( $request, $columns, &$bindings )
	{
		...
		if ( $where !== '' ) {
			$where = 'WHERE ' . $where;
			if($this->filter_language !== false) $where .= " AND ";
		}
		else if($this->filter_language !== false) $where .= " WHERE ";
	 ...
	 */
	public function simple ( $request, $type, $lang, $columns )
	{
		$bindings = array();
		...
		if (in_array($type, array('product', 'category', 'information'))) {
			$select_cols = "`".implode("`, `", self::pluck($columns, 'db'))."`";
			$table = '`'.DB_PREFIX . $type . "_description` LEFT JOIN `" . DB_PREFIX . $type . "` USING(" . $primaryKey . ")";
			$this->filter_language = $lang;
			$default_order = 'name';
      if ($this->config->get('mlseo_ml_mode')) {
        $extra_select = ",(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = CONCAT('".$type."_id=' , ".$type."_id) AND language_id = '".(int) $lang."' LIMIT 1) AS seo_keyword";
      } else {
        $extra_select = ",(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = CONCAT('".$type."_id=' , ".$type."_id) LIMIT 1) AS seo_keyword";
      }
			if($type == 'information') {
				$default_order = 'title';
			}
			...
			$default_order = 'name';
			$extra_select = ",(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = CONCAT('".$type."_id=' , m.".$type."_id) LIMIT 1) AS keyword";
		} elseif (in_array($type, array('common', 'special'))) {
			$select_cols = "`query`, `keyword`, `url_alias_id`";
      ...
      if ($this->config->get('mlseo_ml_mode')) {
        ...
        $this->filter_language = false;
        if ($type == 'common') {
          $extra_where = "WHERE query LIKE 'route=%'";
        } elseif ($type == 'special') {
          $extra_where = "WHERE query NOT LIKE 'route=%'
                       AND query NOT LIKE 'product_id=%'
                       AND query NOT LIKE 'category_id=%'
                       AND query NOT LIKE 'information_id=%'
                       AND query NOT LIKE 'manufacturer_id=%'";
